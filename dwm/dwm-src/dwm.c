...
#define XRDB_LOAD_COLOR(R, V)                                                  \
  if (XrmGetResource(xrdb, R, "Dwm.Color", &type, &value) == True ||           \
...
enum { SchemeNorm, SchemeSel, SchemeWinNorm, SchemeWinSel }; /* color schemes */
enum {
  NetSupported,
  NetWMName,
  NetWMState,
...
void focus(Client *c) {
  ...
  if (c) {
    ...
    attachstack(c);
    grabbuttons(c, 1);
    XSetWindowBorder(dpy, c->win, scheme[SchemeWinSel][ColBorder].pixel);
    setfocus(c);
  } else {
...
void manage(Window w, XWindowAttributes *wa) {
  ...
  XSetWindowBorder(dpy, w, scheme[SchemeWinNorm][ColBorder].pixel);
...
void unfocus(Client *c, int setfocus) {
  if (!c)
    return;
  grabbuttons(c, 0);
  XSetWindowBorder(dpy, c->win, scheme[SchemeWinNorm][ColBorder].pixel);
  if (setfocus) {
    XSetInputFocus(dpy, root, RevertToPointerRoot, CurrentTime);
    XDeleteProperty(dpy, root, netatom[NetActiveWindow]);
  }
...
char xrdb_colors[16][8] = {
    ...
    "#e359a2", /* 3: yellow/accent 3 */
    "#ffa82e", /* 4: blue/accent 4 */
    "#c34f92", /* 5: magenta/accent 5 (default highlight) */
    "#e82a2a", /* 6: cyan/accent 6 */
    "#fff0ed", /* 7: normfgcolor (default fg text) */
    "#7c8a91", /* 8: normbordercolor (default inactive border) */
    "#b35287", /* 9: bright red */
    "#ffb882", /* 10: bright green */
    "#e38fbc", /* 11: bright yellow */
